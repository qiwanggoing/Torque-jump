...
class GO2OmniJumpLandingTorqueCfg(GO2OmniJumpCurriculumTorqueCfg):
    ...
    class commands(GO2OmniJumpCurriculumTorqueCfg.commands):
        ...
        # the history line -- same rule here). An advance fires on a hit ANYWHERE in the challenge band
        # [far_frac*b, b], including its near end, so b only stops once 0.6*b = reach, i.e. b = 1.67x reach.
        # Measured: best at iter ~1750 (hit 0.78, reward 23.4, dx_mean 0.74); dx_mean crossed 0.82 -> hit
        # 0.69 -> 0.15 by iter 2500 and the bounds FROZE there (p90 0.92 / mean 0.86 / min 0.68, unchanged
        # for 3000 iters); landing-reward sum 2.4 -> 0.6, squat_qualified 0.92 -> 0.19, and at iter 4590
        # flight went 0.89 -> 0.01 in one iteration with noise_std 0.04 left to rediscover.
...
class GO2OmniJumpLandingTorqueCfgPPO(GO2OmniJumpCurriculumTorqueCfgPPO):
    ...
    class runner(GO2OmniJumpCurriculumTorqueCfgPPO.runner):
        ...
        # step_down 0.18), so the ceiling climbs slowly by design. 5000 was sized for a FIXED band, and
        # was already cutting mid-climb: the flat line's Delta_air was +0.017 at iter 2300 and +0.254 by
        # 5000, with the hip desaturating 75-83% -> 30-36% over the same stretch.
